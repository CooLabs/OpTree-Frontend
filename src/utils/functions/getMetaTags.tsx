...
const getMetaTags = ({
  ...
  const meta = {
    title: `${title} • OpTree` ?? OPTREE_APP_NAME,
    description: description || OPTREE_APP_DESCRIPTION,
    image: image ?? OG_IMAGE,
    url: isVideo
      ? `${OPTREE_WEBSITE_URL}/watch/${pubId}`
      : `${OPTREE_WEBSITE_URL}/channel/${handle}`
  }
  ...
  let defaultMeta = `<title>${meta.title}</title>
              ...
              <meta property="og:site_name" content="${OPTREE_APP_NAME}" />
              ...
              <meta name="twitter:site" content="@${OPTREE_TWITTER_HANDLE}" />
  ...
  if (isVideo && publication) {
    const embedUrl = `${OPTREE_EMBED_URL}/${pubId}`
    // TODO: add `hasPart`
    const schemaObject = {
    ...
    defaultMeta += `<meta property="og:video" content="${embedUrl}" />
      ...
      <link rel="alternate" type="text/xml+oembed" href="${OPTREE_API_URL}/oembed?format=xml&id=${pubId}" title="${title}" />
      <link rel="alternate" type="application/json+oembed" href="${OPTREE_API_URL}/oembed?format=json&id=${pubId}" title="${title}" />
