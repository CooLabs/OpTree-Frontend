...
import { IS_MAINNET, OPTREE_APP_NAME, OPTREE_DONATION_ADDRESS } from 'utils'
...
const Splits: FC<Props> = ({ submitContainerRef }) => {
  ...
  const isIncludesDonationAddress =
    splitRecipients.filter((el) => el.recipient === OPTREE_DONATION_ADDRESS)
      .length > 0
  ...
  const addDonation = () => {
    const splits = splitRecipients
    splits.push({ recipient: OPTREE_DONATION_ADDRESS, split: 2 })
    setSplitRecipients([...splits])
    scrollToSubmit()
  }
      ...
      {splitRecipients.map((splitRecipient, i) => (
        <div className="flex gap-1.5" key={i}>
          <Input
            ...
            spellCheck="false"
            suffix={
              splitRecipient.recipient === OPTREE_DONATION_ADDRESS
                ? OPTREE_APP_NAME
                : ''
            }
            disabled={splitRecipient.recipient === OPTREE_DONATION_ADDRESS}
            validationError={
      ...
      <div className="flex items-center justify-between space-x-1.5 pt-1">
        <div className="flex items-center space-x-1">
          ...
          {!isIncludesDonationAddress && (
            <Tooltip
              content={`Help ${OPTREE_APP_NAME} continue to grow by adding a donation.`}
              placement="top"
            >
              <button
                type="button"
