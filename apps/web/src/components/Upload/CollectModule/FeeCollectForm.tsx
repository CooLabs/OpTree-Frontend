...
const FeeCollectForm: FC<Props> = ({
  ...
  const splitRecipients = uploadedImage.collectModule.multiRecipients ?? []
  ...
  const {
    ...
    defaultValues: {
      referralPercent: Number(uploadedImage.collectModule.referralFee || 0),
      currency:
        uploadedImage.collectModule.amount?.currency ?? WMATIC_TOKEN_ADDRESS,
      amount: uploadedImage.collectModule.amount?.value || '0',
      collectLimit: uploadedImage.collectModule.collectLimit || '0'
    }
  })

  useEffect(() => {
    if (uploadedImage.collectModule.collectLimitEnabled) {
      register('collectLimit')
    } else {
      unregister('collectLimit')
    }
  }, [uploadedImage.collectModule, register, unregister])
  ...
  const validateInputs = (data: FormData) => {
    ...
    const { isFeeCollect } = uploadedImage.collectModule
        ...
        <>
          <div>
            ...
            <select
              ...
              {...register('currency')}
              value={uploadedImage.collectModule.amount?.currency}
              onChange={(e) => {
                setCollectType({
